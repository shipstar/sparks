...
Elm.Fibonacci.make = function (_elm) {
   ...
   var fibonacci = function (n) {
      return function () {
         var fibonacci$ = F2(function (n,
         acc) {
            return _U.cmp(n,
            2) < 1 ? acc : A2(fibonacci$,
            n - 1,
            A2($List._op["::"],
            $List.head(acc) + function ($) {
               return $List.head($List.tail($));
            }(acc),
            acc));
         });
         return $List.reverse(A2(fibonacci$,
         n,
         _L.fromArray([1,1])));
      }();
   };
...
Elm.FibonacciBars.make = function (_elm) {
   ...
   var color = F2(function (total,
   index) {
      return function () {
         var colorVal = (255 / total | 0) * index;
         return A3($Color.rgb,
         colorVal,
         255 - colorVal,
         0);
   ...
   var main = function () {
      var fibs = $Fibonacci.fibonacciWithIndexes(10);
      var total = $List.length(fibs);
      var colorFunc = color(total);
